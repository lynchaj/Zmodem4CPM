...
char *name, *prev;
{
	...
	if(Dcreate(prev) != 0)
	{
		log2("Trouble trying to create subtree\n");
		fprintf(STDERR,"%s\n", prev);
		return ERROR;
	}
#ifndef REMOTE
...
int a, b, c;
{
	...
	{	/* verbose 2 is normal for us */
		...
		if(vdebug && (logf != (FILE *)NULL))
		{
			fprintf(logf, f, a, b, c);
			fprintf(logf, "\n");
			fflush(logf);
		}
...
/* longs and pointers are not integers */
/* VARARGS1 */
vfile2(f, a, b, c)
long a, b, c;
register char *f;
