...
int purpose;
{
   ...
   killlabel();
   if (purpose == QUIT) {
      printf("%sAre you SURE you want to exit HModem? (Y/N) <N>  ",Stline);
      c = toupper(getchar());
      printf(Endline);
      killlabel();
      if (c != 'Y')
   ...
   printf("\033y7\033G");   /* normal keypad and no graphics */
   cls();
   if (purpose == QUIT)
      exit(0);
   reset(Invokdrive);
   putlabel("HModem Configuration Menu");
   exec("hconfig.com","hm.com");
}
