...
#define lmalloc malloc
	...
	{
		//int speed = getbaud();
		int speed = 0; // FIXME: Hardcoded to 19200 for now.
	...
	if (Verbose > 2)
	{
		if ((logfile = fopen(RLOGFILE, "a")) == (FILE *)NULL)
		{
...
int wcrx()
{
	...
	sendchar=Crcflg?WANTCRC:NAK;
        ...
        unsigned char *usecbuf = (unsigned char *) secbuf;
	...
	for (;;)
	{
		...
		if (sectcurr==((sectnum+1) &Wcsmask))
		{
			...
			if (putsec(usecbuf, cblklen)==ERROR)
				return ERROR;
...
get2:
			...
			if ((sectcurr+(oldcrc=readline(1)))==Wcsmask)
			{
				...
				log3("Sector number garbled 0%o 0%o\n",
				 sectcurr, oldcrc);
		}
...
#ifdef RDEBUG
			if(logfile != (FILE *)NULL)
				fprintf(logfile,  "Incoming: %s %ld %lo %o\n",
