 ...
 */
	...
	if (Verbose > 2)
	{
		if ((logfile = fopen(SLOGFILE, "a"))== (FILE *)NULL)
		{
			fprintf(STDERR, "Can't open log file %s\n",SLOGFILE);
			RETURN(0200);
		}
		fprintf(logfile, "Progname=%s\n", Progname);
		vdebug = 1;
		fflush(logfile);
	}
#endif
	...
	if ( !Modem) {
		if (!Command && !Quiet && Verbose != 1)
		{
			...
			if(Verbose > 2)
			{
				fprintf(logfile, "sz: %d file%s requested:\n",
					npats, npats>1?"s":"");
				for ( agcnt=npats, agcv=patts; --agcnt>=0; )
				{
					fprintf(logfile, "%s ", *agcv++);
				}
				fprintf(logfile, "\n");
				fflush(logfile);
			}
...
long flen;
{
	...
	for (;;) {
		if (flen <= (charssent + 896L))
			thisblklen = 128;
		if ( !filbuf((unsigned char *)txbuf, thisblklen))
			break;
...
int cseclen;	/* data length of this sector to send */
{
	...
	if (Verbose>1)
	{
		...
		if(Verbose > 2)
		{
			fprintf(logfile, "\rBlock %d %dK ", Totsecs, (Totsecs>>3) );
			fflush(logfile);
		}
#endif
	}
...
int whence;
{
	...
	if (pos < (vpos -TXBSIZE +1024)) {
		...
		while (vpos < pos) {
		    n = (int)stread(fptr, (unsigned char *)secbuf, (long)TXBSIZE);
		    vpos += n;
#ifdef SDEBUG
	...
	if (pos > vpos) {
		...
		while (vpos <= pos) {
			...
			n = (int)stread(fptr, (unsigned char *) txbuf, (long)m);
			vpos += n;
			if (n < m) {
				BEofseen = 1;
				break;
...
int timeout;
{
	...
	if (Verbose>3) {
		...
		byt[1] = 0;
		fprintf(logfile, "Timeout=%d Calling alarm(%d) ", timeout, c);
		fflush(logfile);
